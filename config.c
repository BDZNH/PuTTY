...
#define HOST_BOX_TITLE "主机名 (IP地址)"
		      ...
		      int protocol, int protcfginfo)
{
    ...
    if (!midsession) {
	ssd->loadbutton = ctrl_pushbutton(s, "加载", 'l',
					  HELPCTX(session_saved),
					  sessionsaver_handler, P(ssd));
	...
	ctrl_radiobuttons(s, "会话登录:", NO_SHORTCUT, 2,
    ...
    ctrl_filesel(s, "Log文件名:", 'f',
		 NULL, TRUE, "选择一个Log文件",
		 HELPCTX(logging_filename),
		 conf_filesel_handler, I(CONF_logfilename));
    ctrl_text(s, "(Log文件名可以包含 &Y, &M, &D 以表示日期,"
	      " &T 表示时间, &H 表示主机名, &P 表示端口)",
	      HELPCTX(logging_filename));
    ctrl_radiobuttons(s, "若Log文件存在则:", 'e', 1,
		      HELPCTX(logging_exists),
		      conf_radiobutton_handler, I(CONF_logxfovr),
		      "覆盖", I(LGXF_OVR),
		      "追加到文件尾", I(LGXF_APN),
		      "每次都询问用户", I(LGXF_ASK), NULL);
    ctrl_checkbox(s, "常常刷新Log文件", 'u',
